...
sudo launchctl setenv NO_PROXY  '127.0.0.1,localhost,.corp.example,.internal.example'
sudo launchctl setenv no_proxy  '127.0.0.1,localhost,.corp.example,.internal.example'
...
sudo sh -c "
  ...
  for var in NO_PROXY no_proxy; do
    /usr/libexec/PlistBuddy -c \"Add :EnvironmentVariables:\$var string 127.0.0.1,localhost,.corp.example,.internal.example\" '$plist' 2>/dev/null || \
    /usr/libexec/PlistBuddy -c \"Set :EnvironmentVariables:\$var 127.0.0.1,localhost,.corp.example,.internal.example\" '$plist'
  done
